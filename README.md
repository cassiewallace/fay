...
- **Authentication:** Token is held in app state (`@State` in the root view); no keychain persistence for this scope. If the token expires, the next API call returns 401 and the user sees the error state with a retry button. A production app would: (1) persist the token in Keychain for session restore, (2) decode the JWT `exp` claim to proactively refresh or prompt re-login before expiry, (3) on 401, clear the token and present the login screen instead of a generic error, and (4) consider refresh tokens if the API supports them.
